...
incident_type = match.group(1).upper() if match else "MARITIME SECURITY"
match = re.search(
r"UKMTO has received.",
text,
re.I
)
description = match.group(0).strip() if match else text
...
location = match.group(1).strip() if match else "UKMTO AREA OF OPERATIONS"
incidents.append({
